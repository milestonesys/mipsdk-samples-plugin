    ...
    public class SensorMonitorControllerItemManager : ItemManager
    {
        ...
        public override Collection<EventGroup> GetKnownEventGroups(CultureInfo culture)
        {
            return new Collection<EventGroup>()
            {
                ...
                {
                    ID = EventGroupId,
                    Name = SensorMonitorDefinition.GetTranslationString("EVENTGROUPNAME", culture)
                }
            };
        }
        ...
        public override string GetItemStatusDetails(Item item, string language)
        {
            ...
            string controlState = SensorMonitorDefinition.GetTranslationString("ITEMSTATUSDETAILS_CONTROLLERSTATE", culture);
            string someCounter = SensorMonitorDefinition.GetTranslationString("ITEMSTATUSDETAILS_SOMECOUNTER", culture);
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                using (XmlWriter xmlWriter = XmlWriter.Create(memoryStream, xmlWriterSettings))
                {
                    ...
                    xmlWriter.WriteAttributeString("detailname", controlState);
                    xmlWriter.WriteElementString("detail_string", GetOperationalState(item).ToString());
                    xmlWriter.WriteEndElement();

                    xmlWriter.WriteStartElement("detail");
                    xmlWriter.WriteAttributeString("detailname", someCounter);
